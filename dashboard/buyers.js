document.addEventListener('DOMContentLoaded', function() {
  ...
  const header = document.querySelector('header');
    // === FETCH USER PROFILE FROM API AND DISPLAY NAME ===
  async function fetchUserProfile() {
    try {
      ...
      const response = await fetch('https://farmlink-api.onrender.com/api/v1/users/profile', {
        method: 'GET',
        headers: {
      ...
      const headerGreeting = document.querySelector('header h2'); // Assuming there's an <h2>Hello, ___</h2>
      if (headerGreeting) {
        headerGreeting.textContent = `Hello, ${username}!`;
      }
  ...
  fetchUserProfile(); // Call it immediately
